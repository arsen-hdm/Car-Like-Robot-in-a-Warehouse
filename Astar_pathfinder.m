...
% function path = Astar_pathfinder(parent,numnodes)
% 
%     bigmatrix = zeros(1,numnodes);
%     bigmatrix(numnodes) = numnodes;
...
function path = Astar_pathfinder(parent, goalNode)
    % starts from the goal node
    path = goalNode;
    currentnode = goalNode;

    % Backtracking until it returns to the first node (ID = 1)
    while currentnode ~= 1
        currentnode = parent(currentnode);
        path = [currentnode, path]; % it's inserted at the head
    end
end